...
Gira server-side (GitHub Actions), quindi niente CORS.

Fase 1 ha trovato: JPM subsidiary (RSSD 852218) ha cet1_ratio 16.084 (troppo
alto, è il subsidiary). structure/full ha rivelato l'RSSD della HOLDING:
1039502 (JPMORGAN CHASE & CO.). Questa fase 2 usa quell'RSSD trovato in
automatico e prova se lo stesso endpoint /banks/{rssd}/ funziona anche per
una holding company.

Filosofia del progetto: meglio "non so ancora" scritto chiaro che un numero
plausibile ma non verificato.

Output: cet1-probe-log.json nel repo.
"""
import json
import os
import sys
...
def main():
    ...
    if isinstance(struct_full, dict):
        ...
        if parents:
            ...
            print(f"→ trovata holding RSSD dalla struttura: {holding_rssd} ({parents[0].get('name')})")
    ...
    if isinstance(results.get("bank_structure_full", {}).get("json"), dict):
        ...
        results["bank_structure_full"]["json"] = {
            "data": {"rssd_id": d.get("rssd_id"), "parents": d.get("parents"), "n_subsidiaries": len(d.get("subsidiaries") or [])}
        }
    ...
    log = {
        ...
        "scope": "probe fase 2 — holding RSSD trovata via structure/full, testata su /banks/{rssd}/",
        "holding_rssd_found": holding_rssd,
        "results": results,
        "cet1_field_hints": hints,
        "next_step": (
            "Guarda 'banks_endpoint_on_holding_rssd' in results: se status 200 "
            "e cet1_ratio vicino a 14.8, l'endpoint /banks/{rssd}/ funziona "
            "uniformemente per bank e holding — pipeline sbloccata. Se 404, "
            "prova 'holding_guess_1'/'holding_guess_2'. Se anche quelli "
            "falliscono, bankregreports probabilmente non espone il dato "
            "holding via API pubblica anche se il sito ne parla — si passa "
            "al piano B (TCE/TA calcolato da FMP)."
        ),
    }
